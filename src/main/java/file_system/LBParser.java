...
public class LBParser {
	...
	/**
	 * Private field to hold a reference to the CellMap used for organizing the rows.
	 */
	...
	/**
	 * Private method to read the workbook into a buffer for easier sorting and extraction of key data.
	 * 
	 * @throws IOException
	 * @throws InvalidFormatException
	 */
	public void readWorkBookIntoBuffer(final File theFile, final int theSelectedStartIndex) throws InvalidFormatException, IOException {
	...
	/**
	 * Public method used to read the potential sorting rows for selection by the user.
	 * 
	 * @throws IOException
	 * @throws InvalidFormatException
	 */
	public List<List<String>> readPotentialSortingRows(final int theTopRows, final File theFile) throws InvalidFormatException, IOException {
	...
	/**
	 * Private method used to create a New Student and return it to the RowBuffer.
	 * 
	 * @param theCurrentRow
	...
	private Student createNewStudent(final Row theCurrentRow) {
		...
		int studentID = Integer.parseInt(theCurrentRow.getCell(1).getStringCellValue().trim());
		...
		Student current = new Student(studentID, name, building, bdate, language, model, grade);
	...
	private void constructCellMap(final List<Student> theRowBuffer) {
		...
		for(int i = 0; i < theRowBuffer.size(); i++) {
			...
			myCellMap.get(currentBuilding).add(createStudentData(theRowBuffer.get(i)));
			evaluateNativeLanguages(theRowBuffer.get(i).getMyLanguage(), currentBuilding);
		}
	}

	/**
	 * Private method used to create a new LanguageMap which will count all the languages in any given school.
	 * 
	...
	/**
	 * Private method to create a String[] representing Student Data that will be representative of StudentData in a future excel spreadsheet.
	 * 
	 * @param student
	...
	/**
	 * Private method to create a new Linked for a school and the corresponding list of String[] which represents a future excel worksheet.
	 * 
	 * @param theCurrentSchool
	...
	/**
	 * Private method to return an array of Strings that represents the current Worksheet Title of the list of String[] that will represent a future
	 * excel worksheet.
	 * 
	 * @param theCurrentSchool
	 * @param currentDay
	...
	private String[] getTitleCells(String theCurrentSchool) {
		return new String[] {theCurrentSchool + "--" + LBDate.getCurrentMonth() + "-" + LBDate.getCurrentDay() + "-" + LBDate.getCurrentYear(), null, null, null, null, null, null};
	}

	/**
	 * Private method to return the Header Cells to the Beginning of the List for a new School.
	 * 
	 * @param theCurrentSchool
	 *        is the Current School Title
	...
	private String[] getHeaderCells(final String theCurrentSchool) {
		return new String[] {HeaderCase.ID_HEADER.text, HeaderCase.NAME_HEADER.text, HeaderCase.BUILDING_HEADER.text, HeaderCase.BIRTHDATE_HEADER.text, HeaderCase.LANGUAGE_HEADER.text, HeaderCase.MODEL_HEADER.text, HeaderCase.GRADE_HEADER.text};
	}
