...
public class LBFileController {
	...
	/**
	 * Public method to close the SLA Workbook accessible to the scope of the ExcelWriter.
	 */
	...
	/**
	 * Public method to set the Index at which the ExcelWriter will began reading from the master workbook.
	 * 
	 * @param theIndex
	 *        is the startIndex.
	 */
	...
	/**
	 * Public method to create an instance of an SLA Workbook should the calling code necessitate it.
	 * 
	 * @param theDate
	 *        is the date used to create a Unique Folder ID.
	 */
	public void createNewOverViewWorkbook(final String theDate) {
		mySLACount = new SLACount(LocalStorage.getSLABlankPath(), LocalStorage.getWorkingDirectory(), theDate);

	}

	/**
	 * Public method to get the path of the folder that contains the other files within a current execution of the application.
	 * 
	...
	/**
	 * Public method used to read and return the top 'n' rows as selected by the User. This will be the first call when the file is selected. When the
	 * User selects one of the top 'n' rows, the workbook will be read. Does not guarantee that the list will not be null.
	 */
	...
	/**
	 * Public method to read the workbook into a buffer using the method of the internal class LBParser.
	 */
