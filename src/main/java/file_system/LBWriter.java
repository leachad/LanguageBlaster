...
public class LBWriter {
	...
	/**
	 * Publicly accessible method that will be called by the LB Parser at the completion of it's reading and sorting operation.
	 */
	public void writeCountData(final Map<String, List<String[]>> theCellMap, final SLACount theSLACount, final Map<String, Set<NativeLanguage>> theLanguageMap) {
		...
		while(keySetIterator.hasNext()) {
			...
			writeSortedRows(currentSchool, currentSheetData, theLanguageMap.get(currentSchool));
	...
	/**
	 * Private method to change the school observed in the file read. Also creates a new instance of a Language Set for counting student languages.
	 * 
	 * @throws IOException
	 */
	private Workbook createNewWorkbook(final String theCurrentSchool) throws IOException {

		Workbook currentBook = new HSSFWorkbook();
		myCurrentFilePath = LocalStorage.myOutputFolder.getAbsolutePath().concat(File.separator).concat(theCurrentSchool.trim()).concat(FileResource.EXT_CONCAT.text + FileResource.XLS.text);

		myOutputStream = new FileOutputStream(myCurrentFilePath);

		final String name = WorkbookUtil.createSafeSheetName(LBDate.getCurrentMonth().concat(FileResource.SPACE.text).concat(LBDate.getCurrentYear()).concat(FileResource.DASH.text + FileResource.DASH.text).concat(theCurrentSchool));
		final Sheet currentSheet = currentBook.createSheet(name);

		final HSSFPrintSetup printSetup = (HSSFPrintSetup)currentSheet.getPrintSetup();
	...
	private void styleTitleRow(final Row theRow, final Workbook theCurrentBook) {
		theCurrentBook.getSheetAt(TOP_SHEET_INDEX).addMergedRegion(new CellRangeAddress(theRow.getRowNum(), theRow.getRowNum(), 0, NUM_COLUMNS - 1));
	}
	...
	/**
	 * Private method to write out all the sorted rows to the workbook created from a portion of the other work book. Does not guarantee sorted order
	 * for the String[]'s
	 */
	private void writeSortedRows(final String theCurrentSchool, final List<String[]> theSheetCellData, final Set<NativeLanguage> theLanguages) {
		...
		try {
			...
			JOptionPane.showMessageDialog(null, BlasterError.CREATE_BOOK_ERROR.text + e.getMessage());
		}
	 ...
	 */
	private void closeStream(Workbook theCurrentBook, final String theCurrentSchool, final Set<NativeLanguage> theLanguageSet) throws IOException {
	...
	/**
	 * Private method to create a language row used for displaying student language data.
	 * 
	 * @param theLanguage
	 *        is the NativeLanguage.
	 */
	private int createLanguageRow(final NativeLanguage theLanguage, final int rowIndex, final int theCurrentTotal, final Workbook theCurrentBook, final String theCurrentSchool) {
		int runningCount = 0;
		final Row newRow = theCurrentBook.getSheetAt(TOP_SHEET_INDEX).createRow(rowIndex);
		newRow.createCell(LANGUAGE_CONSTANT).setCellValue(theLanguage.getLanguageLabel());

		runningCount = theLanguage.getCountLabel() + theCurrentTotal;
		newRow.createCell(LANGUAGE_CONSTANT + 1).setCellValue(theLanguage.getCountLabel());

		if(mySLACount != null) {
			mySLACount.writeToSheet(theLanguage.getLanguageLabel(), theCurrentSchool, theLanguage.getCountLabel());
	 ...
	 */
	private void createCountRow(final int theTotalCount, final Workbook theCurrentBook, final int lastRowNum) {
	...
	/**
	 * Private method to count the languages and display them at the bottom of the page for totalling and data.
	 * 
	 * @param theSheet
	 *        is the sheet passed in for adding this data.
	 */
	private void countLanguages(final Workbook theCurrentBook, final String theCurrentSchool, final Set<NativeLanguage> theLanguageSet, final int lastRowNum) {
		...
		while(iterator.hasNext()) {
			...
			totalCount = createLanguageRow(language, rowCounter, totalCount, theCurrentBook, theCurrentSchool);
