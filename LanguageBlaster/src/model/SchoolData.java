...
public class SchoolData {
	...
	public SchoolData(final Set<NativeLanguage> theLanguageSet,
			final String theSchoolName, final byte[] theBytes,
			final String[] theEmailAddresses, final String theFileAddress,
	 ...
	 * for easy distribution via email.
	 */
	...
	/**
	 * Public method to return the appropriately URI encoded subject, body and
	 * message etc.
	 * 
	 * @return the URI String to supply to the message body.
	 */
	public String getURIString() {

		StringBuilder addresses = new StringBuilder();
		String[] teacherEmails = getEmailAddresses();

		for (int i = 0; i < getEmailAddresses().length; i++) {
			addresses.append(teacherEmails[i].trim()).append(";").append("%20")
					.trimToSize();
		}

		String URI = "mailto:" + addresses.toString().trim() + "?subject="
				+ getCurrentMonth() + "%20Counts"
				+ "&body=Attached%20is%20your%20current%20" + getCurrentMonth()
				+ "%20class%20list" + "%20for%20" + getEmailName();
